...
生成式人工智能在生成文本、图像和代码方面表现出色。以往，智能体与人类的交互层有以ChatGPT为代表的文本聊天式窗口和A2UI这样的实时生成的UI层。现在，是时候让它用于生成动态的、具有情感表达能力的虚拟形象了。人类的注意力非常宝贵，相比文本对话框、带有按钮交互的UI、和CLI，在很多情况下，与数字人进行交互会很有吸引力。我们发布 **ADA (Agent Driven Avatar)** 项目，聚焦于虚拟形象的规范化与跨平台表达。

ADA 提供了完整的智能体虚拟形象解决方案，采用**双层协议架构**：

1. **动态控制层协议（ADA Protocol）** - 实时虚拟形象渲染和控制的声明式 JSON 协议
2. **静态配置层协议（Avatar.md Format）** - 定义完整数字人人格的综合配置格式

### 动态控制层协议（ADA Protocol）
...
## Identity
- Appearance: White coat, stethoscope
...
```
静态配置层协议 (Avatar.md)     动态控制层协议 (ADA Protocol)
        ↓                              ↓
    "谁是这个人？"                  "这个人现在做什么？"
        ↓                              ↓
  定义外观、性格、知识            控制表情、动作、语音
```

**实际场景**：
1. 用静态配置层协议创建一个医生角色："Sarah 医生，心理学专家，温和友善"
2. 用动态控制层协议让她表现："微笑着说'你好'，用手指向健康建议卡片"

**为什么需要双层架构？**
- **静态层**保证数字人的一致性和可识别性（跨平台、跨会话保持相同身份）
- **动态层**实现智能体的表现力和响应能力（根据对话实时调整）
- **分层设计**降低带宽消耗（静态配置只需加载一次，动态消息轻量高频）

📖 **快速链接**：
- [静态配置层协议规范](./AVATAR_SPEC.md) - Avatar.md 完整格式定义
...
- 多个智能体在同一场景中交互
- 虚拟形象需要区分和协调
- 表情和动作需要同步，不同智能体驱动的数字人之间的表情、语气的识别可以采用更省资源的元数据进行交互

## 解决方案：双层协议架构

ADA 提供了一种**声明式、分层的虚拟形象解决方案**，统一不同平台的数字人控制标准：

- **静态配置层**：定义数字人的身份、人格和能力（静态配置层协议）
  - 对于3D渲染平台：指定模型资源和外观参数
  - 对于生成式平台（可灵、HeyGen等）：提供外观描述，转换为生成提示词

- **动态控制层**：实时控制数字人的表现和交互（JSON 协议）
  - 对于3D渲染平台：直接驱动骨骼动画和表情混合
  - 对于生成式平台：转换为驱动API参数（如HeyGen的表情控制）
  - 对于人形机器人：转换为电机控制序列

类似于 A2UI 对用户界面的处理方式，ADA 通过双层架构实现了灵活性和一致性的平衡：
...
└─────────────────┼───────────────────────────────┘
        ...
        ▼         ▼         ▼         ▼         ▼
    ┌─────┐  ┌─────┐  ┌────────┐  ┌──────────┐  ┌──────────┐
    │ Web │  │移动端│  │ VR/AR  │  │人形机器人│  │生成平台  │
    └─────┘  └─────┘  └────────┘  └──────────┘  └──────────┘
        │         │         │          │             │
        └─────────┼─────────┼──────────┼─────────────┘
                  ▼         ▼          ▼             ▼
        客户端渲染器 / 数字人驱动平台 / 数字人生成平台
    (本地3D渲染) (驱动机器人) (可灵/HeyGen等，通过提示词/图片/视频生成)
```
...
2. **多平台适配层**
   - **客户端实时渲染**：Web (Three.js)、移动端 (Unity)、VR/AR
   - **物理设备驱动**：人形机器人（宇树G1等）
   - **生成式AI平台**：
     - **短视频生成平台**（如可灵）：通过提示词、参考图片生成数字人视频
     - **数字人驱动平台**（如HeyGen）：通过图片或视频创建数字人，实时驱动其说话和表情

3. **资源与渲染方式**
   - **客户端实时渲染器**：使用本地3D模型和动画资源，直接解析动态控制层协议
   - **数字人驱动平台适配器**（HeyGen等）：
     - 将静态配置层的外观描述转换为平台创建参数
     - 将动态控制层协议转换为平台驱动API调用
   - **生成式平台适配器**（可灵等）：
     - 将静态配置层转换为视觉描述提示词
     - 将动态控制层（表情、动作、语气）转换为生成提示词
     - 适用于预录制视频场景
...
动态控制层协议 (标准JSON)
         ...
         ├──> Three.js渲染器 ──> 浏览器3D显示
         ├──> Unity渲染器 ──> 移动App
         ├──> HeyGen适配器 ──> HeyGen API ──> 驱动数字人
         └──> 可灵适配器 ──> 提示词生成 ──> 视频生成
```

### 动态控制层协议消息体系

动态控制层协议采用**分类消息**架构，支持高效的增量更新和精确的时序控制。
...
- **框架无关**：支持多种渲染和生成方式
  - **实时3D渲染**：Three.js, Unity, Unreal, ARKit, ARCore
  - **生成式AI平台**：可灵（Kling）、HeyGen、D-ID 等
  - **物理设备驱动**：宇树G1等人形机器人
- **广泛适配**：从低端移动设备到高端 VR 头显、人形机器人、视频生成平台都能良好运行
- **渐进增强**：根据设备/平台能力提供不同质量级别和实现方式
  - 客户端3D渲染：实时交互、低延迟
  - 生成式平台：高质量视觉效果、适合预录制场景
- **一致性保证**：同一数字人在不同平台上保持可识别的核心特征（通过静态配置层协议）
...
动态控制层协议定义了虚拟形象的多层状态架构：

**协议分层说明**：
1. **静态配置层（Avatar.md）**：定义数字人的基础属性，如相貌、年龄、身高、性格等不变或少变的信息
2. **动态表现层（ExpressionState/ActionSequence/SpeechMessage）**：定义瞬时的表情、动作、语音等表现形式
3. **消息传输层（ADAMessage）**：承载控制指令，作用于静态层和表现层，驱动数字人产生动态表现
...
客户端需实现标准渲染器接口，用于接收和处理动态控制层协议消息，将虚拟形象渲染到屏幕或驱动机器人：

> **说明**：ADARenderer 是客户端（如 Web 应用、移动 App、VR 设备、人形机器人等）需要实现的核心接口。智能体通过 动态控制层协议发送 JSON 消息，客户端的渲染器接收这些消息，然后调用对应的方法（如 `initialize` 初始化数字人、`applyUpdates` 更新表情、`performAction` 执行动作等）来控制虚拟形象的显示或机器人的动作。
...
动态控制层协议与 A2UI 的深度集成可以创造丰富的交互体验：
...
#### 规划支持（部分为概念/设计）🔎
- **Web 实时渲染**: Three.js, Babylon.js
- **移动端**: Unity (iOS/Android)
- **框架**: React Three Fiber
- **数字人生成平台**:
  - **可灵（Kling）**: 通过提示词、参考图片生成数字人短视频
  - **HeyGen**: 通过图片或视频创建数字人，支持实时驱动
  - **原理**: 将静态配置层协议（外观描述）和动态控制层协议（动作、表情、语音）转换为生成平台的提示词和驱动参数
...
- **3D 工具**: Blender 插件
- **实时通信**: WebRTC 集成
- **更多生成平台**: D-ID, Synthesia, Runway 等
...
**方案**：使用可灵或HeyGen等生成平台 + ADA双层协议

**步骤**：
1. **静态配置层**定义品牌数字人形象（product-presenter.avatar.md）
```markdown
# 产品展示专员 - 小林
...
- Appearance:
  ...
  - Age: 25-30
  - Style: 商务休闲，亲和力强
  - Reference Image: ./images/presenter-reference.jpg
```
...
```json
{
  "type": "avatar.speech",
  "timestamp": 1738329600000,
  "agent_id": "product-presenter-001",
  "platform_adapter": "kling",  // 或 "heygen"

  "speech": {
    "text": "大家好！今天为您介绍我们的新款智能手表...",
    "prosody": {
      "emotion": "enthusiastic",
      "emphasis_words": [3, 7],  // "新款"、"智能手表"
  ...
  "platform_specific": {
    "kling": {
      "prompt": "一位25-30岁的女性产品展示专员，商务休闲装扮，面带微笑热情地介绍产品，强调'新款'和'智能手表'时表情更加生动",
      "reference_image": "product-presenter.jpg",
...
### ADA 双层协议 vs. VRM/glTF
- **VRM/glTF**：静态 3D 模型格式，只描述外观
- **ADA 静态配置层**：定义完整数字人人格（外观+性格+知识）
- **ADA 动态控制层**：实时控制虚拟形象的表现和交互
- **关系**：ADA 可以使用 VRM/glTF 作为底层 3D 模型格式
...
动态控制层协议使用情感计算模型将对话上下文映射到虚拟形象表现：
...
以下为可对接/参考的生态与平台（不代表官方合作）：

**技术伙伴**
- 🌐 **A2UI 项目**：无缝的 UI + Avatar 集成（动态控制层协议与 A2UI 协同）
- 🔗 **A2A 协议**：多智能体通信标准（支持多智能体虚拟会议）
- 🎮 **Unity Technologies**：游戏引擎集成
- 🎨 **Ready Player Me**：虚拟形象资源库（可用于静态配置层）
...
ADA（包括静态配置层协议和动态控制层协议）采用 **Apache 2.0** 开源许可证，您可以自由使用、修改和分发。
...
### Q: ADA 双层协议与 A2UI 的关系？
A: ADA 和 A2UI 是互补的。A2UI 处理界面元素，ADA 的双层协议（静态配置层+动态控制层）处理虚拟形象。它们可以协同工作，提供完整的智能体用户体验。

### Q: 需要 3D 建模经验吗？
A: 通常不需要。基础场景可使用预制的虚拟形象库，通过 JSON 配置即可使用。

### Q: 支持自定义虚拟形象吗？
A: 支持。您可以导入 VRM/glTF 格式的自定义模型，并映射到 ADA 的控制系统。

### Q: 性能要求高吗？
A: ADA 支持多级质量与渐进增强。从低端移动设备（2D sprite）到高端 VR（完整 3D）都有适配策略，但最终表现取决于平台实现与资源质量。

### Q: 如何保证内容安全？
A: 客户端完全控制资源库和渲染。智能体只能从预批准的库中选择，无法注入任意内容。

### Q: 商业使用需要付费吗？
A: 当前版本开源免费，包括商业用途；如未来策略调整将以许可证与版本说明为准。
...
**推荐学习路径**：
1. 📖 理解双层协议架构：
   - 静态配置层协议（Avatar.md 格式）- 定义数字人身份
   - 动态控制层协议（JSON 消息）- 控制实时表现
2. 📄 阅读 [动态控制层协议 JSON 格式设计分析](./JSON_DESIGN_ANALYSIS.md)
3. 🎮 试玩 [在线演示](https://ada.dev/playground)
4. 💻 克隆仓库并运行示例
5. 🛠️ 集成到您的智能体项目
